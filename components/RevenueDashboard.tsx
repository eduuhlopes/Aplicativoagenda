...
const RevenueCard: React.FC<{ title: string; amount: number; colorClass: string }> = ({ title, amount, colorClass }) => {
    ...
    return (
        <div className={`bg-white p-6 rounded-xl shadow-md border-l-4 ${colorClass}`}>
            <h3 className="text-lg font-semibold text-[var(--text-body)] mb-2">{title}</h3>
            <p className="text-4xl font-extrabold text-[var(--text-dark)]">{formattedAmount}</p>
        </div>
    );
};
...
const RevenueDashboard: React.FC<RevenueDashboardProps> = ({ projectedRevenue, monthlyRevenue }) => {
    return (
        <div className="flex flex-col">
             <h2 className="text-3xl font-bold text-[var(--text-dark)] text-center mb-4 flex items-center justify-center">
                Painel Financeiro
                <ChartBarIcon />
            </h2>
            <div className="space-y-6">
                <RevenueCard
                    ...
                    colorClass="border-[var(--info)]"
                />
                <RevenueCard
                    title="Faturamento do Mês"
                    amount={monthlyRevenue}
                    colorClass="border-[var(--success)]"
                />
            </div>
             <div className="mt-6 text-center text-sm text-[var(--primary)] italic">
